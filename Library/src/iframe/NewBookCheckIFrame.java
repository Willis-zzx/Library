...
public class NewBookCheckIFrame extends JInternalFrame {
     ...
     * 
     */
    private static final long serialVersionUID = 7632004667401319730L;
    ...
    private String addkucun;
    private JFormattedTextField orderDate;
    private Operater user = LoginUI.getUser();

    JRadioButton radioButton2;
    JRadioButton radioButton1;
    private String[] columnNames = { "图书编号", "定购日期", "定购数量", "操作员", "是否验收", "折扣", "图书类别", "图书名称", "作者", "译者", "出版社", "出版日期", "图书价格" ,"库存"};

    private Object[][] getFileStates(List<OrderAndBookInfo> list) throws SQLException {
        Object[][] results = new Object[list.size()][columnNames.length];
        for (int i = 0; i < list.size(); i++) {
            OrderAndBookInfo order = list.get(i);
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NewBookCheckIFrame frame = new NewBookCheckIFrame();
	 ...
	 * @throws SQLException 
	 */
	public NewBookCheckIFrame() throws SQLException {
        super();
        setClosable(true);
        setIconifiable(true);
        setAutoscrolls(true);
        ...
        Object[][] results = getFileStates(DBUtil.selectBookOrder());
    ...
    class CheckActionListener implements ActionListener {
        ...
        public void actionPerformed(final ActionEvent e) {
            if (radioButton2.isSelected()) {
                ...
                addkucun = orderNumber.getText();
                int i = DBUtil.UpdateCheckBookOrder(ISBNs,Integer.parseInt(addkucun));
                if (i == 1) {
                    JOptionPane.showMessageDialog(null, "验收成功！");
                    Object[][] results = null;
					try {
						results = getFileStates(DBUtil.selectBookOrder());
                ...
                JOptionPane.showMessageDialog(rootPane, "您选择的图书已经进行过验收，请选择其他图书进行验收");
            }
